...
export const loadMessages = async (classId: string, userId: string) => {
  ...
  const localMessages = await loadMessagesFromLocalStorage(classId);
  store.dispatch(setMessages({ classId, messages: localMessages }));

  const isConnected = await checkInternetConnection();

  if (isConnected) {
    // If online, fetch from Firestore to update
    subscribeToMessages(classId, userId, (messages) => {
      // This callback will be called when new messages are received
      saveMessagesToLocalStorage(classId, messages);
    });
  }
};
export const syncOfflineMessages = async () => {
  ...
  const allClasses = store.getState().class.classes;
  for (const classItem of allClasses) {
    const localMessages = await loadMessagesFromLocalStorage(classItem.id);
    const offlineMessages = localMessages.filter(
      (msg) => !msg.id || msg.id.length < 20
    ); // Assuming Firestore IDs are longer
    ...
    for (const message of offlineMessages) {
      try {
        ...
        const updatedMessages = localMessages.map((msg) =>
          msg === message ? { ...msg, id: docRef.id } : msg
        );
        await saveMessagesToLocalStorage(classItem.id, updatedMessages);
      } catch (error) {
